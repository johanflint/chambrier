...
@Component
class NumberPropertyChangedReducer : Reducer {
    override fun reduce(event: Event, state: State): State {
        if (event is NumberPropertyChanged) {
            ...
            if (device == null) {
                logger.warn { "Received NumberPropertyChanged event for unknown device '${event.deviceId}': $event" }
                return state
            }

            val property = device.properties[event.propertyId]
            if (property == null || property !is NumberProperty) {
                logger.warn { "Received NumberPropertyChanged event for device '${device.id}' (${device.name}), but for unknown property '${event.propertyId}': $event" }
                return state
            }
